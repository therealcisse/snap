 ...
 * selection by version runs only inside sub-replays and `materializeVersion`, where dropping
 * is the point.
 */
...
import { type Tree, assertPrefixFree, equalBytes, namespaceConflicts } from './tree.ts';
 ...
 * canonical integration order `log` prints reversed. Sub-replay integrations are not part of a
 * top-level replay's sequence.
 */
 ...
 * tree, the §6.4 warning set, and the integration order of the top-level integrations.
 *
...
export function replayRepository(repository: Repository, hooks?: ReplayHooks): ReplayResult {
  ...
  const tree = replaySelection(core, pending, warnings, sequence);
  return { tree, warnings: sortWarnings(warnings), sequence };
}
...
export function materializeVersion(repository: Repository, version: Version): Tree {
  ...
  return replaySelection(core, selected, new Map<string, WarningPair>(), []);
}

/**
 * The shared §6.1–§6.2 walk: integrate every patch of `pending` from the empty tree in
 * least-ready order, recording warning pairs into `warnings` and integration order into
 * `sequence`. The top-level call keeps both; a sub-replay passes maps and arrays it discards
 * (§6.2). All walks share one memo: a base materialized by either is the same tree.
 */
